...
for /F "eol=; tokens=*" %%i in (filelist.txt) do echo ^<table style="display:inline;"^>^<tr^>^<th^>^<img src=%%i width=256px onclick=CopyToClipboard("%%i")^>^</th^>^</tr^>^<tr^>^<th^>%%i^</th^>^</tr^>^</tr^>^</table^> >>index.html
